...
uintptr_t utils::find_pattern(const uintptr_t base, const size_t size,
                              const char* bmask, const char* szmask) {
    for (size_t i = 0; i < size; ++i)
        if (data_compare(reinterpret_cast<const char*>(base + i), bmask,
                         szmask))
            return base + i;
...
uintptr_t utils::find_pattern_section(const uintptr_t base,
                                      const char* szsection, const char* bmask,
                                      const char* szmask) {
    if (!base || !szsection || !bmask || !szmask)
        return 0;

    const auto* psection = pe::get_section_header(base, szsection);
    return psection ? find_pattern(base + psection->VirtualAddress,
                                   psection->SizeOfRawData, bmask, szmask)
                    : 0;
}

uintptr_t utils::find_pattern_km(const wchar_t* szmodule, const char* szsection,
                                 const char* bmask, const char* szmask) {
...
uintptr_t utils::get_ntos_base() {
    ...
    const auto RtlPcToFileHeader = reinterpret_cast<f_RtlPcToFileHeader>(

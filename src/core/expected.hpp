...
//
// A simple expected<T, E> implementation for kernel mode
// No dynamic memory allocation - value stored inline
//
// Usage:
//   expected<int, ErrorCode> foo() {
//       if (error_condition)
//           return unexpected(ErrorCode::SomeError);
//       return 42;
//   }
//
//   auto result = foo();
...
public:
    ...
    constexpr expected(T&& val) : has_val_(true), value_(static_cast<T&&>(val)) {}
    
    // Error constructor
    constexpr expected(const unexpected<E>& err) : has_val_(false), error_(err.error) {}
    constexpr expected(unexpected<E>&& err) : has_val_(false), error_(static_cast<E&&>(err.error)) {}
    ...
    [[nodiscard]] constexpr T value_or(U&& default_val) const& {
        return has_val_ ? value_ : static_cast<T>(static_cast<U&&>(default_val));
    }

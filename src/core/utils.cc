...
uintptr_t find_pattern(uintptr_t base, size_t size, const char* hex_pattern) {
    ...
    if (!parse_hex_pattern(hex_pattern, pattern_bytes, pattern_wildcards, &pattern_len)) {
        return 0;
    }
    ...
    for (size_t i = 0; i <= size - pattern_len; i++) {
        bool match = true;
        const unsigned char* data = reinterpret_cast<const unsigned char*>(base + i);
...
uintptr_t find_pattern_section(uintptr_t base, const char* section_name, const char* hex_pattern) {
    ...
    return find_pattern(base + section->VirtualAddress, section->SizeOfRawData, hex_pattern);
}
...
uintptr_t get_kernel_module_base(const wchar_t* module_name) {
    ...
    for (auto* entry = PsLoadedModuleList->Flink;
         entry != PsLoadedModuleList;
         entry = entry->Flink) {
        auto* ldr_entry = CONTAINING_RECORD(entry, KLDR_DATA_TABLE_ENTRY, InLoadOrderLinks);

...
void write_vmcs_ctrl_fields(vcpu* const cpu) {
  ...
  // The guest clock is maintained through the TSC offset so a world switch
  // does not show up as elapsed guest time.
  ...
  // Record which optional controls the processor actually accepted so the
  // fields they guard are never written unconditionally.
  cpu->cached.preemption_timer_supported =
      (vmx_vmread(VMCS_CTRL_PIN_BASED_VM_EXECUTION_CONTROLS) >> 6) & 1;
  cpu->cached.perf_global_ctrl_supported =
      (vmx_vmread(VMCS_CTRL_PRIMARY_VMEXIT_CONTROLS) >> 12) & 1;
  ...
  vmx_vmwrite(VMCS_CTRL_CR0_GUEST_HOST_MASK,
    cpu->cached.vmx_cr0_fixed0 | ~cpu->cached.vmx_cr0_fixed1 | CR0_CACHE_DISABLE_FLAG | CR0_NOT_WRITE_THROUGH_FLAG);
  vmx_vmwrite(VMCS_CTRL_CR4_GUEST_HOST_MASK, cpu->cached.vmx_cr4_fixed0 | ~cpu->cached.vmx_cr4_fixed1);
  vmx_vmwrite(VMCS_CTRL_CR0_READ_SHADOW, __readcr0());
  // CET: CR4.CET is left exactly as Windows set it and the VMCS CET load
  // controls (VM-entry bit 20, VM-exit bit 28) stay clear. Root mode and the
  // guest are the same kernel: the guest changes the shadow stack pointer in
  // non-root mode with every call and return, so the VMCS could only hold a
  // snapshot and loading it on a transition would clobber the live value.
  // Leaving both controls clear keeps the one CET state intact across the
  // world switch. Supervisor enforcement is the part that cannot work in that
  // model, and it is filtered off on write (see emulate_wrmsr).
  vmx_vmwrite(VMCS_CTRL_CR4_READ_SHADOW, __readcr4() & ~CR4_VMX_ENABLE_FLAG);

  // 3.24.6.7
  // Every CR3 write resets process-specific EPT views, including PCID no-flush writes.
  ...
  // 3.24.6.4, 3.24.7.2, 3.24.8.2
  // Snapshot the performance counters the guest may be using as a clock, and
  // restore the guest values on vm-entry. `hide_vm_exit_overhead` adjusts the
  // restored values so a world switch is not observable through them.
  ...
  // The MSR lists exist for the timing compensation; with the compensation
  // switched off they are empty so the guest counters pass through untouched.
  // Fault-class exceptions are intercepted only for the bring-up trace; the
  // real machine delivers them without exiting.
  vmx_vmwrite(VMCS_CTRL_EXCEPTION_BITMAP,
    hv::fault_trace_enabled() ? hv::fault_trace_exception_bitmap : 0);

  vmx_vmwrite(VMCS_CTRL_VMEXIT_MSR_STORE_COUNT, hv::timing_hidden() ? 4 : 0);
  vmx_vmwrite(VMCS_CTRL_VMEXIT_MSR_STORE_ADDRESS,
      MmGetPhysicalAddress(&cpu->msr_exit_store).QuadPart);
  vmx_vmwrite(VMCS_CTRL_VMENTRY_MSR_LOAD_COUNT, hv::timing_hidden() ? 2 : 0);
  ...
  if (cpu->cached.perf_global_ctrl_supported)
...
void write_vmcs_host_fields(vcpu const* const cpu) {
  ...
  // The ASM frame and C++ guest_context both start on a 16-byte boundary.
  auto const rsp = ((reinterpret_cast<size_t>(cpu->host_stack)
    + host_stack_size) & ~0b1111ull);

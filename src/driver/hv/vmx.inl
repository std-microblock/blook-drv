...
inline void vmx_vmwrite(uint64_t const field, uint64_t const value) {
  if (__vmx_vmwrite(field, value)) fatal_root_error();
}

// VMREAD instruction
inline uint64_t vmx_vmread(uint64_t const field) {
  uint64_t value{};
  if (__vmx_vmread(field, &value)) fatal_root_error();
  return value;
}
...
// Forward declarations: the skip helpers below report an unusable length by
// injecting #GP into the guest, which is deterministic and debuggable, instead
// of resuming it somewhere unpredictable.
inline void inject_hw_exception(uint32_t vector);
inline void inject_hw_exception(uint32_t vector, uint32_t error);

// Same, for the cases where VMCS_VMEXIT_INSTRUCTION_LENGTH cannot be trusted
// and the length was decoded from the emulated instruction instead.
inline void skip_instruction(uint32_t const length) {
  // increment RIP
  auto const old_rip = vmx_vmread(VMCS_GUEST_RIP);
  auto new_rip       = old_rip + length;
...
// Defined in exit-handlers.cpp: decodes the length of the intercepted
// instruction from the guest's own bytes, because the value the processor
// reports for it cannot be trusted for every exit class.
...
inline void skip_instruction() {
  auto const decoded = decode_guest_instruction_length();
  if (decoded) {
    skip_instruction(decoded);
    return;
  }
  auto const reported = static_cast<uint32_t>(vmx_vmread(VMCS_VMEXIT_INSTRUCTION_LENGTH));
  if (reported == 0 || reported > 15) {
    // Nothing that can be resolved to a plausible instruction length; this
    // is not expected to be reachable.
    fatal_root_error();
  }
  skip_instruction(reported);
}

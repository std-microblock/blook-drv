...
PIMAGE_SECTION_HEADER pe::get_section_header(const uintptr_t image_base,
                                             const char* section_name) {
    if (!image_base || !section_name)
        return nullptr;

    const auto pimage_dos_header =
        reinterpret_cast<PIMAGE_DOS_HEADER>(image_base);

    if (pimage_dos_header->e_magic != IMAGE_DOS_SIGNATURE)
        return nullptr;

    const auto pimage_nt_headers = reinterpret_cast<PIMAGE_NT_HEADERS>(
        image_base + pimage_dos_header->e_lfanew);
    ...
    for (USHORT i = 0; i < NumberOfSections; ++i) {
        if (!_strnicmp(reinterpret_cast<const char*>(&psection->Name[0]),

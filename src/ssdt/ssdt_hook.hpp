...
class SsdtHook {
   ...
   public:
    ...
    // Assign a non-capturing lambda / function pointer as the hook
    // implementation. Example: hook << [](Args...){ /* ... */ };
    template <typename Fn>
    SsdtHook& operator<<(Fn fn) {
        // requires non-capturing lambda or function pointer (convertible to
        // function pointer)
...
class SsdtHookManager {
   ...
   public:
    ...
    // `hook_obj->enable()`.
    [[nodiscard]] core::Result<SsdtHook&> hook_by_syscall_name(
        const char* syscall_name, HookType type = HookType::Ssdt);

    // Check if an index is already hooked
    [[nodiscard]] bool is_hooked(unsigned short index,
                                 HookType type = HookType::Ssdt) const;

    // Unhook all installed hooks
    [[nodiscard]] core::VoidResult unhook_all();

    // Get a routine's current address
    [[nodiscard]] void* get_routine(unsigned short index,
                                    HookType type = HookType::Ssdt) const;
   ...
   private:
    ...
    [[nodiscard]] bool do_hook(unsigned short index, void* dest,
                               void** original, HookType type);
    [[nodiscard]] bool do_unhook(unsigned short index, void* original,
                                 HookType type);
    ...
    // Find hook by index
    [[nodiscard]] core::Result<SsdtHook&> find_hook(unsigned short index,
                                                    HookType type);
